test_that("doubletFinder works", {

  # Make test deterministic
  set.seed(0)
  # Err if wrong length, type, or if any missingness
  testthat::expect_error(
    doubletFinder(pbmc_small, PCs = 1:4, pN = 0.25, pK = 0.01, nExp = 5, reuse.pANN = FALSE, sct=FALSE,
                     annotations = "blah")
  )
  testthat::expect_error(
    doubletFinder(pbmc_small, PCs = 1:4, pN = 0.25, pK = 0.01, nExp = 5, reuse.pANN = FALSE, sct=FALSE,
  ...
  testthat::expect_output(
    pbmc_small_dubs <- doubletFinder(pbmc_small, PCs = 1:4, pN = 0.25, pK = 0.01, nExp = 5, reuse.pANN = FALSE, sct=FALSE,
                                        annotations = as.character(pbmc_small$RNA_snn_res.1))
  )
  testthat::expect_output(
    pbmc_small_dubs <- doubletFinder(pbmc_small, PCs = 1:4, pN = 0.5, pK = 0.01, nExp = 5, reuse.pANN = FALSE, sct=FALSE,
                                        annotations = as.character(pbmc_small$RNA_snn_res.1))
  )
